...
import { LinkGameModal } from '@/components/LinkGameModal';
import { 
  User, 
  Trophy, 
  Gamepad2, 
...
export default function ProfilePage() {
  ...
  const handleUnlinkGame = async (gameId: string, gameName: string) => {
    if (!confirm(`¿Estás seguro de desvincular tu cuenta de ${gameName}?`)) return;

    const res = await api.delete(`/profile/games/${gameId}`);
    if (res.success) {
      setFeedback({ type: 'success', message: `Cuenta de ${gameName} desvinculada.` });
      loadFullProfile();
    } else {
      setFeedback({ type: 'error', message: res.error?.message || 'Error al desvincular la cuenta.' });
    }
  };

  const linkedGameCodes = gameProfiles.map((g) => g.game_code);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div className="p-4 rounded-xl bg-[#0B0C10] border border-emerald-500/30 text-center space-y-2 relative group hover:border-emerald-500/60 transition-all">
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-emerald-400 font-semibold">Supercell ID Validado</p>
            </div>
          </div>
      ...
      {/* 3. TWO-COLUMN LAYOUT: ACADEMIC DATA & LINKED GAMES */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        
        {/* LEFT: ACADEMIC PROFILE FORM (7 cols) */}
        <div className="lg:col-span-7 arena-card p-6 sm:p-8 space-y-6">
          <div className="flex items-center gap-3 border-b border-white/10 pb-4">
            <div className="w-10 h-10 rounded-xl bg-[#457B9D]/20 text-[#457B9D] flex items-center justify-center">
              <GraduationCap className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-lg font-black text-white">Información Académica</h2>
              <p className="text-xs text-[#8E92A4]">Datos validados para torneos interuniversitarios</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSaveProfile} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                rows={3}
                placeholder="Escribe sobre tu estilo de juego, roles favoritos o trayectoria competitiva..."
            ...
            <button
              type="submit"
              disabled={isSaving}
              className="btn-primary py-2.5 px-6 text-xs flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
            >
              {isSaving ? (
                <>
        ...
        {/* RIGHT: LINKED SUPERCELL GAMES (5 cols) */}
        <div className="lg:col-span-5 arena-card p-6 sm:p-8 space-y-6">
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-amber-500/20 text-amber-400 flex items-center justify-center">
              ...
              <div>
                <h2 className="text-lg font-black text-white">Cuentas Oficiales</h2>
                <p className="text-xs text-[#8E92A4]">Vinculación Supercell ID</p>
              </div>
            </div>
          ...
          {gameProfiles.length === 0 ? (
            <div className="p-8 text-center bg-[#0B0C10] rounded-xl border border-white/5 space-y-4">
              <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center mx-auto text-[#8E92A4]">
                <Gamepad2 className="w-6 h-6" />
              </div>
              <div className="space-y-1">
                <p className="text-sm font-bold text-white">No tienes cuentas vinculadas</p>
                <p className="text-xs text-[#8E92A4] max-w-xs mx-auto">
                  Vincula tu Player Tag oficial para habilitar tu inscripción a los torneos de la liga.
                </p>
              </div>
              <button
                onClick={() => setIsModalOpen(true)}
            ...
            <div className="space-y-4">
              {gameProfiles.map((game) => (
                <div 
                  key={game.id}
                  className="p-5 bg-[#0B0C10] rounded-xl border border-white/10 space-y-4 relative group"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                        game.game_code === 'CLASH_ROYALE' ? 'bg-[#E63946]/20 text-[#E63946]' : 'bg-[#457B9D]/20 text-[#457B9D]'
                      }`}>
                        {game.game_code === 'CLASH_ROYALE' ? <Swords className="w-5 h-5" /> : <Gamepad2 className="w-5 h-5" />}
                      </div>
                      <div>
                        <p className="text-sm font-bold text-white">{game.game_name}</p>
                        <p className="text-xs text-[#8E92A4]">
                          Apodo Oficial: <span className="text-white font-semibold">{game.in_game_name}</span>
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => handleUnlinkGame(game.id, game.game_name)}
                        title="Desvincular cuenta"
                        className="p-1.5 text-[#5A5E73] hover:text-[#E63946] rounded-lg hover:bg-white/5 transition-colors cursor-pointer"
                      >
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-white/5">
                    <div className="p-2 bg-[#15161E] rounded-lg">
                      <span className="text-[#8E92A4] block text-[10px]">PLAYER TAG</span>
                      <span className="font-mono font-bold text-[#A8DADC]">{game.player_tag}</span>
                    </div>

                    <div className="p-2 bg-[#15161E] rounded-lg">
                      <span className="text-[#8E92A4] block text-[10px]">COPAS OFICIALES</span>
                      <span className="font-bold text-amber-400 flex items-center gap-1">
                        <Trophy className="w-3 h-3" />
                        {game.trophies.toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
      ...
      <LinkGameModal
        ...
        onSuccess={() => {
          setFeedback({ type: 'success', message: '¡Cuenta de juego vinculada exitosamente con la API oficial!' });
          loadFullProfile();
        }}
        alreadyLinkedGames={linkedGameCodes}
      />
